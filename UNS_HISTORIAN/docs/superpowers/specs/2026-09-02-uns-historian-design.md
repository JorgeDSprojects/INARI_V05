...
5. **Dedup check per reading**: compare the reading's normalized `payload` (or `raw_payload` if JSON parsing failed) against the cache entry for `topic`. If identical, drop it (no insert, no cache update). Otherwise, update the cache and append `{time, topic, payload, raw_payload, qos, retain}` to an in-memory flush buffer.
   ...
   - **Dedup is applied per MQTT message, not per reading**: for a list-payload message (multiple readings in one publish), the whole ordered set of readings is compared as a unit against the last message received on that topic — this correctly collapses a retained/QoS-1 replay of an unchanged list to zero new rows, instead of cycling through each element as a separate change. **Known limitation**: on ingestor restart, the warm-up cache (`db.load_last_values`) can only reconstruct the *last individual reading* per topic from flat row storage (there is no message-grouping column), not the last full list — so a list-topic can produce one spurious full-duplicate batch immediately after a restart before self-correcting on the next real change. Bounded and self-healing; a full fix (persisting message boundaries) is out of scope for this milestone.
