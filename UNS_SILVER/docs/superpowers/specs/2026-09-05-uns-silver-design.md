...
- **Nested-KPI dot-path reconciliation.** `app/catalog.py` registers each KPI under its bare key (e.g. `fleet_health_score`), but `app/flatten.py` emits dot-paths for nested values (e.g. `fleet_health_score.value`). If the real `_analytical` payload nests KPI values inside an object, those readings will never match the bare-key catalog entry and will land as `signal_type='unknown'` rather than `'kpi'`. The `_descriptive.analytical` payload shape is still an unconfirmed assumption (see the plan's Global Constraints), so no workaround is guessed here — revisit once the shape is confirmed with the team publishing it.
- **Poison-pill row isolation in the Silver normalizer.** A single bronze row that raises during `process_batch` aborts the whole batch, so the watermark never advances and the same rows are refetched forever. The one collision known to be reachable — two `_descriptive` messages for a topic sharing an identical bronze `time`, violating `UNIQUE (topic, signal_key, effective_since)` — is neutralised by the upsert in `db.apply_catalog_changes`, and any exception is now caught and rolled back by `main._safe_process_until_caught_up` so the process survives. General row-by-row isolation with a quarantine table (as `UNS_HISTORIAN`'s ingestor has) for *other* failure classes remains future hardening.
