...
export function NodeTypesView() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setLoadError("Failed to load node types");
    }
  }, []);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setSaveError("Failed to save node type");
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setSaveError("Failed to save schema");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setSaveError("Failed to delete node type");
    }
  };
      ...
      <div className="w-96 border-r border-border flex flex-col overflow-hidden bg-surface shrink-0">
        ...
        {showForm && (
          <div className="px-5 py-4 border-b border-border space-y-3 bg-surface-subtle">
            ...
            {saveError && <p className="text-danger text-xs font-mono mt-1">{saveError}</p>}
          </div>
        )}

        <div className="flex-1 overflow-y-auto">
          {loadError ? (
            <div className="flex items-center justify-center h-full text-danger text-sm py-12">
              {loadError}
            </div>
          ) : nodeTypes.length === 0 ? (
            ...
            <div
              ...
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') setSelected(nt); }}
              role="button"
              tabIndex={0}
              className={`w-full text-left px-5 py-3.5 border-b border-border-subtle flex items-start justify-between group hover:bg-surface-subtle cursor-pointer ${selected?.id === nt.id ? "bg-surface-subtle" : ""}`}
            >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    <button onClick={handleSaveEdit} disabled={savingEdit || !!editSchemaError}
                      className="px-4 py-2 text-sm bg-ink text-white rounded disabled:opacity-50 self-start">
                      {savingEdit ? "Saving…" : "Save schema"}
                    </button>
                    {saveError && <p className="text-danger text-xs font-mono">{saveError}</p>}
                  </div>
                </div>
              ) : (
