...
async def update_cell(line_id: str, cell_id: str, body: CellUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if name_changing:
        old_desc = await build_cell_topic(obj, db, "_descriptive")
        old_info = await build_cell_topic(obj, db, "_informative")
    ...
    desc_topic = await build_cell_topic(obj, db, "_descriptive")
    info_topic = await build_cell_topic(obj, db, "_informative")
